...
def user_context_line() -> str:
    """Return a small context line (date/time + optional location) for the model."""

    now = datetime.now().astimezone()
    dt = now.isoformat(timespec="seconds")

    # Location is intentionally optional: users can set it via an env var.
    location = os.environ.get("MICRO_AGENT_LOCATION") or os.environ.get("LOCATION") or "unknown"

    return f"System date/time: {dt} | Location: {location}"
...
def model_list_text(*, cfg: dict[str, Any], api_key: Optional[str], base_url: str, limit: int = 15) -> str:
    ...
    current = (cfg.get("micro_agent", {}).get("model") or "").lower().strip()
    lines = ["Models (live):"]
    for m in models:
...
def try_set_model(
    *,
    ...
    cfg["micro_agent"]["model"] = matched_id
